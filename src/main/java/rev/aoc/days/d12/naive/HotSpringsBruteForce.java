...
public class HotSpringsBruteForce extends AocSolution<Long> {
  ...
  public static long getNumPossibleArrangements(Pair<char[], int[]> rowSchematic) {
    ...
    if (springs.length == 0) {
      return brokenLengths.length == 0
          ? 1
          : 0; // if we still have some broken lengths at this point then the solution is invalid
    }
  ...
  public static List<Pair<char[], int[]>> parse(List<String> lines) {
    ...
    for (int i = 0; i < lines.size(); i++) {
      ...
      Pair<char[], int[]> springsAndBrokenCounts =
          Pair.of(rowSchematic[0].toCharArray(), brokenCounts);
