...
public abstract class HotSprings extends AocSolution<Long> {
  ...
  public static long getNumPossibleArrangements(Pair<char[], int[]> rowSchematic) {
    ...
    return getNumPossibleArrangements(
        chars, lengths, dp_table, chars.length - 1, lengths.length - 1);
  }

  private static long getNumPossibleArrangements(
      char[] chars, int[] lengths, long[][] dp_table, int charIndex, int lengthIndex) {
  ...
  public List<Pair<char[], int[]>> parse(List<String> lines) {
    ...
    for (int i = 0; i < lines.size(); i++) {
      ...
      Pair<char[], int[]> springsAndBrokenCounts =
          toSchematic(rowSchematic[0].toCharArray(), brokenCounts);
