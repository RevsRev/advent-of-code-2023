...
public class Graph
{
    ...
    public Pair<GraphNode, Pair<Long, Long>> extractCycle(char[] steps, GraphNode start)
    {
        ...
        while (true)
        {
            ...
            if (previousTraversals.getOrDefault(node.getName(), new HashMap<>()).containsKey(indexModulo))
            {
    ...
    private Pair<GraphNode, Pair<Long, Long>> atomize(char[] steps, GraphNode start, long startIndex, long endIndex)
    {
        GraphNode node = start;
