...
public abstract class ClumsyCrucible extends AocSolution<Long> {
  ...
  protected Long solveImpl() throws Exception {
    ...
    GraphPath<Vec4, DefaultWeightedEdge> pathBetween =
        DijkstraShortestPath.findPathBetween(graph, new Vec4(0, 0, 0, 0), end);
  ...
  protected SimpleDirectedWeightedGraph<Vec4, DefaultWeightedEdge> toGraph(int[][] cityBlock) {
    ...
    SimpleDirectedWeightedGraph<Vec4, DefaultWeightedEdge> graph =
          ...
          if (i + 1 < height) {
            if (0 <= k && k <= max - 1) {
              DefaultWeightedEdge edge =
                  graph.addEdge(new Vec4(i, j, k, 0), new Vec4(i + 1, j, k + 1, 0));
              graph.setEdgeWeight(edge, cityBlock[i + 1][j]);
            }
            if (Math.abs(k) >= min) {
              DefaultWeightedEdge edge =
                  graph.addEdge(new Vec4(i, j, 0, k), new Vec4(i + 1, j, 1, 0));
              graph.setEdgeWeight(edge, cityBlock[i + 1][j]);
            }
          }
          if (j + 1 < width) {
            if (0 <= k && k <= max - 1) {
              DefaultWeightedEdge edge =
                  graph.addEdge(new Vec4(i, j, 0, k), new Vec4(i, j + 1, 0, k + 1));
              graph.setEdgeWeight(edge, cityBlock[i][j + 1]);
            }
            if (Math.abs(k) >= min) {
              DefaultWeightedEdge edge =
                  graph.addEdge(new Vec4(i, j, k, 0), new Vec4(i, j + 1, 0, 1));
              graph.setEdgeWeight(edge, cityBlock[i][j + 1]);
            }
          }
          if (i - 1 >= 0) {
