...
public class RangeMap {
  ...
  public final List<Range<Long>> resolve(Range<Long> input) {
    ...
    for (int i = lIndex; i < rIndex; i++) {
      ...
      int rangeEnd = rIndex;
      if (rangeStart % 2 == 0) { // trivial mapping up to (but not including) the next index
        retval.add(Range.closed(l, ranges.get(rIndex) - 1));
      } else {
        retval.add(
            Range.closed(
                map(ranges.get(lIndex), l), map(ranges.get(rIndex), ranges.get(rIndex) - 1)));
      }
    }
